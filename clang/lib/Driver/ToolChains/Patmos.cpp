...
{
  //----------------------------------------------------------------------------
  // link with start-up files crt0.o, crtbegin.o, crtend.o
...
//  LinkInputs.push_back(Args.MakeArgString(getLibPath("lib/libm.a")));
...
//  // Allow unused code to not be linked
//  LinkInputs.push_back("--only-needed");
    ...
    llvm::opt::ArgStringList &LinkInputs) const
{
  ...
  // The optimizer might have removed some librt functions, therefore,
  // we link them again
  LinkInputs.push_back(Args.MakeArgString("--override=" + getLibPath("lib/librt.a")));
                               ...
                               const char *LinkingOutput) const
{
  ...
  const char *link2Out = CreateOutputFilename(C, Output, "link2-", "bc", false);
  ...
  if (!ConstructOptJob( *this, C, JA, Output, Inputs, optOut, link2Out,  Args)) {
    optOut = link2Out;
  }
  ...
  PrepareLink3Inputs(Args, optOut, Link3Inputs);
  ConstructLLVMLinkJob(*this, C, JA, Output, Inputs, link3Out, Link3Inputs, Args);

  ////////////////////////////////////////////////////////////////////////////
  // build LLC command
  const char *llcOut = CreateOutputFilename(C, Output, "link2-", "bc", false);
